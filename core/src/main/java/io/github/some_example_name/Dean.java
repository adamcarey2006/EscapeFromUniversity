...
public class Dean {
	...
	public Dean(float x, float y, Player player) {
		this.position = new Vector2(x, y);
		this.startPosition = new Vector2(x, y); // store the starting position of the dean
		this.player = player;
		this.velocity = new Vector2();
	...
	public void update(float delta) {
		...
		// Move if gameScreen is null (testing) or cell is not blocked
		if (gameScreen == null || !gameScreen.isCellBlocked(newX, newY)) {
	...
	/**
	 * Reset the dean to its starting position when the player is caught or to the
	 * other side of the map
	 */
	public void resetToStart(int caughtNumber) {
		if (caughtNumber % 2 == 0) {
			// if the number of times caught by the dean is even send them to a new positon
			// than their starting, otherwise send them to the start
